local function map(mode, lhs, rhs)
	vim.keymap.set(mode, lhs, rhs, { silent = true })
end

	-- Telescope
-- local status, telescope = pcall(require, "telescope.builtin")
-- if status then
-- 	map("n", "<C-p>", telescope.find_files)
-- 	map("n", "<leader>fg", telescope.live_grep)
-- 	map("n", "<leader>fb", telescope.buffers)
-- 	map("n", "<leader>fh", telescope.help_tags)
-- 	map("n", "<leader>fs", telescope.git_status)
-- 	map("n", "<leader>fc", telescope.git_commits)
-- else
-- 	print("Telescope not found")
-- end

-- FZF (replaces Telescope)
local status, fzf = pcall(require, "fzf-lua")
if status then
	map("n", "<C-p>", fzf.files)
	map("n", "<leader>fg", fzf.live_grep)
	map("n", "<leader>fb", fzf.buffers)
	map("n", "<leader>fh", fzf.tags)
	map("n", "<leader>fs", fzf.git_status)
	map("n", "<leader>fc", fzf.git_commits)
else
	print("FZF not found")
end

-- hide search
map("n", "<esc>", "<CMD>noh<CR>")

-- Save
map("n", "<leader>w", "<CMD>update<CR>")

-- Quit
map("n", "<leader>q", "<CMD>q<CR>")

-- Windows
-- map("n", "<leader>ñ", "<CMD>vsplit<CR>")
-- map("n", "<leader>p", "<CMD>split<CR>")

-- NeoTree
map("n", "<leader>n", "<CMD>Neotree filesystem reveal right<CR>")
map("n", "<leader>o", "<CMD>Neotree focus<CR>")

-- Buffer
map("n", "<TAB>", "<CMD>bnext<CR>")
map("n", "<S-TAB>", "<CMD>bprevious<CR>")

-- Terminal
-- map("n", "<leader>th", "<CMD>ToggleTerm size=10 direction=horizontal<CR>")
-- map("n", "<leader>tv", "<CMD>ToggleTerm size=80 direction=vertical<CR>")

-- Markdown Preview
map("n", "<leader>m", "<CMD>MarkdownPreview<CR>")
map("n", "<leader>mn", "<CMD>MarkdownPreviewStop<CR>")

-- Window Navigation
map("n", "<C-h>", "<C-w>h")
map("n", "<C-l>", "<C-w>l")
map("n", "<C-k>", "<C-w>k")
map("n", "<C-j>", "<C-w>j")

-- Resize Windows
map("n", "<C-Left>", "<C-w><")
map("n", "<C-Right>", "<C-w>>")
map("n", "<C-Up>", "<C-w>+")
map("n", "<C-Down>", "<C-w>-")

-- Go to Definition
map("n", "gd", ":lua vim.lsp.buf.definition()<cr>")
-- Find References
map("n", "gR", ":lua FzfLua.lsp_references()<cr>")
-- Get Info
map("n", "K", ":lua vim.lsp.buf.hover()<cr>")

-- format only selected buffer
map("n", "<leader>ff", ":lua vim.lsp.buf.format()<cr>")

-- diagnostics
map("n", "gn", ":lua vim.diagnostic.goto_next()<cr>")
map("n", "gp", ":lua vim.diagnostic.goto_prev()<cr>")

